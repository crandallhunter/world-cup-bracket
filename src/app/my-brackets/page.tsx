...
import { getAnonId } from '@/lib/storage/brackets';
...
export default function MyBracketsPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      loadSubmissionsForAddress(getAnonId());
    }
  }, [loadSubmissionsForAddress]);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-text-secondary text-sm mt-1">
            {submittedBrackets.length} bracket{submittedBrackets.length !== 1 ? 's' : ''} submitted
          </p>
        </div>
        <Link
          href="/bracket"
