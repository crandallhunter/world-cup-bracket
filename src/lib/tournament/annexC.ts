// ─── FIFA 2026 Annexe C — Third-placed teams combinations table ────────────
// Source: FIFA World Cup 26 Regulations (March 2026)
//   https://digitalhub.fifa.com/m/636f5c9c6f29771f/original/FWC2026_regulations_EN.pdf
//   PDF pages 41-49 (printed pages 80-96), heading "ANNEXE C — COMBINATIONS
//   FOR EIGHT BEST THIRD-PLACED TEAMS".
//
// This file is AUTO-GENERATED by scripts/parse-annex-c.py from the FIFA PDF.
// Do not hand-edit. If FIFA publishes a revision, re-run the extraction.
//
// Structure:
//   Key   = sorted 8-letter string of advancing group labels (e.g. "ABCDEFGH").
//   Value = 8-tuple of group labels, one per R32 slot, in the order FIFA
//           publishes them: [1A, 1B, 1D, 1E, 1G, 1I, 1K, 1L]
//           which in this app corresponds to:
//             index 0 → M7  (opponent of A1)
//             index 1 → M13 (opponent of B1)
//             index 2 → M10 (opponent of D1)
//             index 3 → M3  (opponent of E1)
//             index 4 → M9  (opponent of G1)
//             index 5 → M6  (opponent of I1)
//             index 6 → M15 (opponent of K1)
//             index 7 → M16 (opponent of L1)
//
// Row count: 495 = C(12, 8). Every possible combination of 8 advancing
// groups out of 12 is covered exactly once.

import type { GroupLabel } from '@/types/tournament';

/** Order in which the Annex-C row is stored: [M7, M13, M10, M3, M9, M6, M15, M16]. */
export const ANNEX_C_SLOT_ORDER = ['M7', 'M13', 'M10', 'M3', 'M9', 'M6', 'M15', 'M16'] as const;

export type AnnexCRow = readonly [
  GroupLabel, GroupLabel, GroupLabel, GroupLabel,
  GroupLabel, GroupLabel, GroupLabel, GroupLabel,
];

export const ANNEX_C_TABLE: Readonly<Record<string, AnnexCRow>> = {
...
/**
 * Look up the Annex-C assignment for a given set of 8 advancing group labels.
 * Returns `null` if the set is not exactly 8 distinct groups from A-L, or if the
 * combination is not in the table (which should never happen — every C(12,8)
 * combination is covered).
 */
export function lookupAnnexC(groups: readonly GroupLabel[]): AnnexCRow | null {
